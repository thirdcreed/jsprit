...
import problem.vehicle.FiniteFleetManagerFactory;
...
import util.SolutionVerifier;
import algorithm.RemoveEmptyVehicles;
import algorithm.ResetAndIniFleetManager;
import algorithm.SearchStrategy;
import algorithm.SearchStrategy.DiscoveredSolution;
import algorithm.SearchStrategyManager;
...
import algorithm.acceptor.AcceptNewRemoveFirst;
import algorithm.acceptor.SchrimpfAcceptance;
import algorithm.acceptor.SolutionAcceptor;
import algorithm.io.VehicleRoutingAlgorithms.TypedMap.AbstractKey;
import algorithm.io.VehicleRoutingAlgorithms.TypedMap.AcceptorKey;
import algorithm.io.VehicleRoutingAlgorithms.TypedMap.InsertionStrategyKey;
import algorithm.io.VehicleRoutingAlgorithms.TypedMap.RuinStrategyKey;
import algorithm.io.VehicleRoutingAlgorithms.TypedMap.SelectorKey;
import algorithm.io.VehicleRoutingAlgorithms.TypedMap.StrategyModuleKey;
...
import algorithm.modules.Gendreau;
...
import algorithm.recreate.listener.VehicleSwitched;
...
import algorithm.ruin.distance.EuclideanServiceDistance;
import algorithm.ruin.distance.JobDistance;
import algorithm.selector.SelectBest;
import algorithm.selector.SelectRandomly;
import algorithm.selector.SolutionSelector;
...
import algorithm.termination.TimeBreaker;
import algorithm.termination.VariationCoefficientBreaker;
...
//				createInitialSolution.setGenerateAsMuchAsRoutesAsVehiclesExist(false);
	...
	private static SearchStrategyModule buildModule(HierarchicalConfiguration moduleConfig, final VehicleRoutingProblem vrp, VehicleFleetManager vehicleFleetManager, 
		...
		if(moduleName.equals("gendreau")){
			...
			if(ruin == null){
				ruin = new RadialRuinStrategyFactory(0.3, new AvgCostsServiceDistance(vrp.getTransportCosts())).createStrategy(vrp);
