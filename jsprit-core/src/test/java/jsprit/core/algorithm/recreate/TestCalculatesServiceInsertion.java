...
public class TestCalculatesServiceInsertion {
	...
	public void setup(){
		...
		vrp = VehicleRoutingProblem.Builder.newInstance().setRoutingCost(costs).addJob(first).addJob(second).addJob(third).build();
		...
		serviceInsertion = new ServiceInsertionCalculator(costs, new LocalActivityInsertionCostsCalculator(costs, activityCosts), constraintManager, constraintManager);
